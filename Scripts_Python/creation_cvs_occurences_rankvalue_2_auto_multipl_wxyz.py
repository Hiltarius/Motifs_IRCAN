...
#modif avc score en multiplication, pas addition
...
def weight(motifw,repetx,occury,rankz,lenmotifl,internrepetl,occurencesl,rankingl):
    ...
    for i in range(len(lenmotifl)):
        newvalue = lenmotifl2[i]^motifw * internrepetl[i]^repetx * occurencesl[i]^occury * (len(rankingl)-i)^rankz
...
def calcul(ww,xx,yy,zz,mbases,accession):
    path = os.getcwd()
    for input_file in os.listdir(path):
        if input_file.endswith(".total"):
            ...
            newrankvalue = weight(ww,xx,yy,zz,lenmotif,internrepet,occurences,ranking)
            #print newrankvalue
            #print motifliste
...
# il faut maintenant creer le csv associe via le dico bigdatabase
    ...
    zz=str(zz)
    ww=str(ww)
    namecsv = "table_"+ww+xx+yy+zz+"_top1000_rankvalues_2__dataslymphoblasto_multipl_wxyz.csv"
    csv = open(namecsv,"w")
    firstline = ("accession;")
    for i in listmotif :
        firstline+=i+";"
    ...
    for key in bigdatabase.keys():
        row=str(key)+";"
        for i in range(lenl):
            row+=bigdatabase[key][listmotif[i]]+";"
        row = row[:-1]
        row+="\n"
        csv.write(row)
...
for w in xrange(0,6) :
    for x in xrange(0,6) :
        for y in xrange(0,6) :
            ...
            calcul(w,x,y,z,mbases,accession)
